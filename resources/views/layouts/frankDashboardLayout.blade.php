@php
    $forceLightTheme = (auth()->user()->role ?? null) === 'admin';
    $preferredTheme = $forceLightTheme ? 'light' : (auth()->user()->theme_preference ?? request()->cookie('scprs_theme', 'light'));
...
<body data-disable-navigation-overlay="1">
    @php
        $pageHeader = match (true) {
            ...
            request()->routeIs('dashboard.themes.index') => [
                'title' => 'Theme Management',
                'subtitle' => 'Manage the system color palette and visual theme settings.',
            ],
